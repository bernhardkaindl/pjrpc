...
import traceback
import pprint
from xjsonrpc.common import Request
...
def serialize_requests() -> Middleware:
    ...
    async def mw(request: Request, context: Any, handler: RpcHandler) -> Any:
        """xjsonrpc Middleware which serializes the execution of RPC methods"""
        async with lock:
...
def log_requests(logger: logging.Logger) -> Middleware:
    ...
    async def mw(request: Request, context: Any, handler: RpcHandler) -> Any:
        """xjsonrpc Middleware which logs the execution of called RPC methods"""
